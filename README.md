...
```bash
# Detached, hands-free (from the dir you want the app in)
# The ralph loop's per-iteration backend is TOOL-AGNOSTIC and separate from
# this host — pin it with BUILD_AGENT=claude|codex, else the host's own CLI,
# else any of opencode/claude/codex on PATH (no preference).
nohup opencode run --command fullstack-builder --auto "your app idea here" > build.log 2>&1 &

# Or in-session
/fullstack-builder your app idea here
```

### Claude Code / Cline / Antigravity / Replit / Cursor / Windsurf

No slash-command wrapper needed — load the `SKILL.md` body directly and give
it the one-line idea; it self-describes its phases. Copy the skill into the
host's skill root (see **Install**).
...
Copy the whole skill directory (`SKILL.md` + the `scripts/`, `docs/`,
`command/`, `skills/`, `bin/` folders) into your host's skill root. Only the
*skill-discovery path* and the optional *surface wrapper* differ per host —
the `SKILL.md` body is identical everywhere. The `skills/` directory contains
the bundled engine family and `bin/` holds the cross-platform ralph
setup/check/guard helpers.
...
| Cline | Cline docs skill root (`.cline/skills`) | inline | Cline/VS Code MCP settings |
| Antigravity / Replit / Cursor / Windsurf | their documented skill root | inline | their documented MCP config / `.mcp.json` |

```bash
# opencode example — copy the whole skill dir into the skill root
cp -r <skill-dir> ~/.config/opencode/skills/fullstack-builder

# opencode-only: slash command
cp <skill-dir>/command/fullstack-builder.md ~/.config/opencode/command/fullstack-builder.md

# Claude Code example — same copy, no slash command needed (inline)
cp -r <skill-dir> ~/.claude/skills/fullstack-builder
```
